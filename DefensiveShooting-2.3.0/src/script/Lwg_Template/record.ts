...
export module record {
    ...
    export enum Engine {
        ...
        'skin和lodeImage' = '如果一张图片没有在场景内，也没有在场景加载的任何一个图集内部，那么打开场景的时候，这张图片是不会被加载的，所以，此时如果需要用到这张图片，只能用sprite中loadImage（url）获取到这张图片，不可以用image.skin=url，也可以用 Laya.loader.load（url）。'
    ...
    export enum PhysicsAndBox2d {
        ...
        'childMove1' = '子节点刚体不会跟着主节点的移动而移动，所以在update里面可以设置子节点跟随，或者建立多个刚体,此方法在相互碰撞中可能会产生问题',

        'childMove2' = '子节点不跟随父节点刚体移动的原因，因为所有的物理Box2d都是在同一个父容器下的，默认为stage，所以全部跟随stage移动， Laya.Physics.worldRoot = sprite,来设置父容器，这样就可以根据父容器移动，但是依然无法解决每个对象都根据父节点移动，因为他们的父节点并不等于！==父容器',
